...
class DatabaseManager:
    ...
    async def insert_token_info(self, token_info: TokenInfo) -> None:
        ...
        logger.debug(f"Token info inserted/ignored: {token_info.symbol} ({token_info.mint})")
    
    async def get_token_info(self, mint: str, platform: str) -> Optional[TokenInfo]:
    ...
    async def insert_trade(
        ...
        row = TradeConverter.to_row(trade_result, mint, timestamp, position_id, trade_type, run_id)
    ...
    async def get_trades_by_mint(self, mint: str) -> List[TradeResult]:
        ...
        async with self.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM trades WHERE mint = ? ORDER BY timestamp",
                (mint,)
            )
            rows = cursor.fetchall()
