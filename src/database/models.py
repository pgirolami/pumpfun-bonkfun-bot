...
class PositionConverter:
    ...
    def from_row(row: Tuple) -> Position:
        ...
        return Position(
            ...
            exit_net_price_decimal=row[12],
            exit_ts=row[13],
            transaction_fee_raw=row[14],
            platform_fee_raw=row[15],
            realized_pnl_sol_decimal=row[16],
            realized_net_pnl_sol_decimal=row[17],
            buy_amount=row[18],
            total_net_sol_swapout_amount_raw=row[19],
            total_net_sol_swapin_amount_raw=row[20],
        )
